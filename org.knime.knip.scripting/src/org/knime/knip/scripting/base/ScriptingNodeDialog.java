...
public class ScriptingNodeDialog extends NodeDialogPane implements
	...
	private final SettingsModelString m_scriptingLanguageModel = ScriptingNodeModel
	...
	private void ScriptingNodeDialogDelayed() {
		...
		final String[] languages = m_scriptService.getLanguages().stream()
				.map((lang) -> {
					return lang.getNames().get(0);
				}).toArray((length) -> {
		...
		m_langSelection.addItemListener((event) -> {
			m_scriptingLanguageModel.setStringValue((String) m_langSelection
					.getSelectedItem());
	...
	protected void saveSettingsTo(final NodeSettingsWO settings)
		...
		m_scriptingLanguageModel.saveSettingsTo(settings);
	...
	protected void loadSettingsFrom(final NodeSettingsRO settings,
		...
		try {
			...
			m_scriptingLanguageModel.loadSettingsFrom(settings);
	...
	private Module compile() {
		...
		try {
			if (m_scriptEngine instanceof JavaEngine) {
				commandClass = ScriptingNodeModel.compile(
						(JavaEngine) m_scriptEngine,
