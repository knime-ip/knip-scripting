...
public class ScriptingNodeDialog extends NodeDialogPane {
	...
	private List<DialogComponent> m_dialogComponents = Collections.emptyList();
	...
	private void ScriptingNodeDialogDelayed() {
		...
		try (final TempClassLoader tempCl = new TempClassLoader(
			...
			try {
				...
				// caused by temporary directory not createable
			}
	...
	protected void saveSettingsTo(final NodeSettingsWO settings)
		...
		try {
			ColumnToModuleItemMappingUtil.fillStringArraySettingsModel(
					m_cimService, m_settings.columnInputMappingModel());
			...
			try (final TempClassLoader tempCl = new TempClassLoader(
				...
				m_compilationResult = m_compiler.compile(
	...
	private void createAutogenPanel() {
		...
		try {
			...
			for (final ModuleItem<?> input : m_compilationResult.inputs()) {
				...
				// If this input is filled by a column mapping
				// don't generate UI for it
